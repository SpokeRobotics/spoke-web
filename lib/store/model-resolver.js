...
/**
 * Recursively expand children from a hierarchical object
 * @param {Object} doc - Store document that may have .children
...
async function expandChildren(doc, parentLocation = { dx: 0, dy: 0, dz: 0, rx: 0, ry: 0, rz: 0 }, maxDepth = 10) {
  ...
  // If doc has children, expand them
  if (Array.isArray(doc.children) && doc.children.length > 0) {
    ...
    for (const child of doc.children) {
      ...
      try {
        ...
        // Recursively expand if child also has children
 ...
 * Handles hierarchical objects with .children by expanding them recursively
...
export async function batchResolveModels(docIds) {
  ...
  for (const $id of docIds) {
    ...
    try {
      ...
      // Check if this is a hierarchical object with children
      if (Array.isArray(doc.children) && doc.children.length > 0) {
