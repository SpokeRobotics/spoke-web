...
import { getEffectiveSlots } from './type-system.js'
import { getNested } from './slot-path.js'
...
export async function resolveModelFromDoc(doc, maxDepth = 5) {
  ...
  // Type reference - follow the chain (support both "type" and "$type")
  const typeRef = doc.type || doc.$type;
  if (typeRef && typeof typeRef === 'string' && typeRef.startsWith('spoke://')) {
    if (maxDepth <= 0) {
      console.warn(`[model-resolver] Max depth reached resolving type chain for ${doc.id || doc.$id}`)
      return null
    }
    
    try {
      const typeDoc = await store.getDoc(typeRef)
      if (typeDoc) {
        return await resolveModelFromDoc(typeDoc, maxDepth - 1)
      }
    } catch (err) {
      console.warn(`[model-resolver] Failed to resolve type ${typeRef}:`, err)
    }
  }
...
export async function resolveTypeChain(doc, maxDepth = 10) {
  ...
  while (depth < maxDepth) {
    const typeRef = current.type || current.$type;
    if (!typeRef || typeof typeRef !== 'string' || !typeRef.startsWith('spoke://')) {
      break
    }
...
/**
 * Recursively expand children from slots in a hierarchical object
 * @param {Object} doc - Store document that may have slots with child references
 * @param {Object} parentLocation - Parent's location to compose with children
...
async function expandChildren(doc, parentLocation = { dx: 0, dy: 0, dz: 0, rx: 0, ry: 0, rz: 0 }, maxDepth = 10) {
  ...
  // Get effective slots for this doc's type
  const typeRef = doc.type || doc.$type
  if (!typeRef) return [] // Leaf node (no type means no slots)
  
  const { byPath: slots } = await getEffectiveSlots(typeRef)
  
  // Iterate through all slots
  for (const [slotPath, slotDef] of Object.entries(slots)) {
    const slotValue = getNested(doc, slotPath)
    if (!slotValue) continue
    
    // Handle both single references and arrays
    const childIds = Array.isArray(slotValue) ? slotValue : [slotValue]
    
    for (const childId of childIds) {
      if (typeof childId !== 'string') continue
      
      try {
        // Fetch the child instance
        const childDoc = await store.getDoc(childId)
        if (!childDoc) continue
        
        // Parse child's location (if present)
        const childLocation = parseLocationString(childDoc.location || '0,0,0,0,0,0')
        
        // Compose with parent location
        const composedLocation = composeLocations(parentLocation, childLocation)
        
        // Recursively expand if child also has slots
        const childExpanded = await expandChildren(childDoc, composedLocation, maxDepth - 1)
        
        if (childExpanded.length > 0) {
          // Child is hierarchical, add its expanded children
        ...
        console.warn(`[model-resolver] Failed to expand child ${childId}:`, err)
      }
    }
  }
 ...
 * Handles hierarchical objects by expanding their slots recursively
 * Can handle both types (for preview) and instances (for rendering)
...
export async function batchResolveModels(docIds) {
  ...
  for (const $id of docIds) {
    ...
    try {
      ...
      // Try to expand children (works for instances with slots)
      const expanded = await expandChildren(doc)
      
      if (expanded.length > 0) {
        // Hierarchical object - process each expanded child
        ...
        // Leaf object - has a model directly
        const model = await getEffectiveModel(doc)
        if (!model || !model.url) continue
